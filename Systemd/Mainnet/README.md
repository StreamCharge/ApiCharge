# ApiCharge Stellar RPC - Bare Metal Deployment (Mainnet)
...
> **Note:** This deployment uses ~4 hours of blockchain history retention (~20-30 GB)
> vs the Docker deployment's 7 days (~160 GB). For applications requiring deep history,
> use the [Docker deployment](../../../Release-Mainnet/) or connect to public RPC endpoints.

## Quick Start

```bash
# 1. Download the release package (or clone this directory)
wget https://github.com/ApiChargeOrg/ApiChargeStellarRPC/releases/latest/download/systemd-mainnet.tar.gz
tar -xzf systemd-mainnet.tar.gz
cd systemd-mainnet
...
To apply new default configs, manually diff and merge:
```bash
diff /etc/apicharge/appsettings.json ./appsettings.json
```
...
Edit `/etc/apicharge/apicharge.env`:
```bash
...
1. Place your certificate at `/etc/apicharge/certs/certificate.pfx`
2. Update `/etc/apicharge/appsettings.json`:
   ```json
   "Certificate": {
     "Path": "/etc/apicharge/certs/certificate.pfx",
     "Password": "your-certificate-password"
   }
   ```
3. Restart: `sudo systemctl restart apicharge`

### Pricing & Routes

Edit `/etc/apicharge/appsettings.json` in the `ApiChargeProxy.Routes` section.

See [ApiCharge Documentation](https://apicharge.com/Documentation/) for details.
...
sudo systemctl stop apicharge apicharge-soroban-rpc apicharge-redis
sudo systemctl disable apicharge apicharge-soroban-rpc apicharge-redis

# Remove systemd units
sudo rm /etc/systemd/system/apicharge*.service
sudo systemctl daemon-reload

# Remove files (optional - preserves data)
sudo rm -rf /opt/apicharge
sudo rm -rf /etc/apicharge

# Remove data (destructive!)
# sudo rm -rf /var/lib/apicharge

# Remove user
sudo userdel apicharge
```
